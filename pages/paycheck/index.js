import Head from 'next/head';
import Container from '../../components/ui/Container';
const { computePaycheck, US_STATES, stateSlug } = require('../../utils/usPaycheckCalculations');

const usd = (n) => `$${Math.round(n).toLocaleString('en-US')}`;

export async function getStaticProps() {
  const states = Object.keys(US_STATES)
    .sort((a, b) => US_STATES[a].name.localeCompare(US_STATES[b].name))
    .map((code) => {
      const st = US_STATES[code];
      const r = computePaycheck({ grossAnnual: 75000, stateCode: code, filingStatus: 'single' });
      return {
        code,
        name: st.name,
        slug: stateSlug(code),
        noTax: st.type === 'none',
        net75: Math.round(r.netAnnual)
      };
    });
  return { props: { states } };
}

export default function PaycheckIndex({ states }) {
  const canonical = 'https://upaman.com/paycheck';
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: 'Take-home pay by US state',
    itemListElement: states.map((s, i) => ({
      '@type': 'ListItem', position: i + 1, name: `Take-home pay in ${s.name}`, url: `https://upaman.com/paycheck/${s.slug}`
    }))
  };

  return (
    <>
      <Head>
        <title>Take-Home Pay by State 2026 | Salary After Taxes in All 50 States | Upaman</title>
        <meta name="description" content="Compare 2026 take-home pay in every US state and D.C. See what a $75,000 salary leaves after federal, FICA, and state taxes, then drill into your state for $40k–$150k tables." />
        <link rel="canonical" href={canonical} />
        <meta property="og:title" content="Take-Home Pay by State 2026 | Upaman" />
        <meta property="og:description" content="Salary after taxes in all 50 states + D.C., with per-state breakdowns." />
        <meta property="og:url" content={canonical} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary" />
        <meta name="twitter:title" content="Take-Home Pay by State 2026 | Upaman" />
        <meta name="twitter:description" content="Salary after taxes in all 50 states + D.C." />
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(itemListSchema) }} />
      </Head>

      <section className="py-8 sm:py-12">
        <Container>
          <div className="mx-auto max-w-[980px]">
            <p className="mb-2 text-xs font-bold uppercase tracking-[0.12em] text-brand-600 dark:text-brand-300">United States · Salary</p>
            <h1 className="font-display text-2xl font-bold tracking-tight text-ink sm:text-3xl dark:text-white">
              Take-Home Pay by State (2026)
            </h1>
            <p className="mt-3 max-w-2xl text-[1.02rem] leading-relaxed text-ink-soft dark:text-slate-300">
              The figure on each card is the estimated annual take-home for a <strong>$75,000 salary</strong> (single
              filer, standard deduction) after federal income tax, Social Security, Medicare, and state tax. Pick your
              state for a full $40k–$150k table, or use the{' '}
              <a href="/us-paycheck-calculator" className="font-medium text-brand-600 underline underline-offset-2 hover:text-brand-700 dark:text-brand-300">US Paycheck Calculator</a>{' '}
              for exact numbers.
            </p>

            <div className="my-6 grid grid-cols-[repeat(auto-fill,minmax(215px,1fr))] gap-2.5">
              {states.map((s) => (
                <a
                  key={s.code}
                  href={`/paycheck/${s.slug}`}
                  className="block rounded-xl border border-slate-200 bg-white px-3.5 py-3 no-underline transition hover:border-brand-300 hover:shadow-soft dark:border-slate-700 dark:bg-slate-800/60 dark:hover:border-brand-700"
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-ink dark:text-white">{s.name}</span>
                    {s.noTax ? <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[0.68rem] font-bold text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">NO TAX</span> : null}
                  </div>
                  <div className="mt-0.5 text-sm text-ink-muted dark:text-slate-400">{usd(s.net75)} on $75k</div>
                </a>
              ))}
            </div>

            <p className="text-sm text-ink-muted dark:text-slate-500">
              Estimates for tax year 2026; local/city taxes excluded unless noted on the state page. Not tax advice.
            </p>
          </div>
        </Container>
      </section>
    </>
  );
}
